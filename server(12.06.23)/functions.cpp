...
QString GetTask4(QString cLogin, QString userAnswer)
{
    // Создаем заданный граф
    QVector<QVector<int>> A = {{1, 4}, {1, 2, 5}, {5, 6}};


    QStringList z = userAnswer.split(" ");
    ...
    for (int i = 0; i < A.size(); i++) {
        ...
        for (int j = 0; j < answer.size(); j++) {
            if (std::find(A[i].begin(), A[i].end(), answer[j]) != A[i].end()) {
                found = true;
                break;
            }
        }
