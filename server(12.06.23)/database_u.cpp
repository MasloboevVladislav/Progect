...
void Database_u::AddAnswer(QString login, QString answer, bool isCorrect) // Добавляем ответы в таблицу базы данных для ведения статистики и последующего использования
{
    QSqlQuery *query = new QSqlQuery(db);

    query->prepare("Update answers Answer = :Answer IsCorrect = :IsCorrect where UserId = :UserId and IsCorrect is NULL");
    query->bindValue(":Answer", answer);
    query->bindValue(":UserId", GetUserId(login));
    query->bindValue(":IsCorrect", isCorrect);
    query->exec();
}

void Database_u::AddVariant(QString login, QString name, QString answer, bool isCorrect) // Добавляем ответы в таблицу базы данных для ведения статистики и последующего использования
{
    QSqlQuery *query = new QSqlQuery(db);

    query->prepare("INSERT INTO answers (Answer, TaskId, UserId, IsCorrect) VALUES(:Answer, :Task, :User, NULL)");
        query->bindValue(":Answer", answer);
...
QString Database_u::GetCurrentTask(QString login)
{
    QSqlQuery *query = new QSqlQuery(db);

    query->prepare("SELECT Name FROM answers INNER JOIN tasks ON answers.TaskId = tasks.id WHERE answers.UserID = :User and IsCorrect is NULL ORDER BY answers.id DESC LIMIT 1");
    query->bindValue(":User", GetUserId(login));
    query->exec();
...
QString Database_u::get_answer(QString login)
{
    QSqlQuery *query = new QSqlQuery(db);

    query->prepare("select Answer from answers where UserID = :User and IsCorrect is NULL ORDER BY id DESC LIMIT 1");
    query->bindValue(":User", GetUserId(login));
    query->exec();
